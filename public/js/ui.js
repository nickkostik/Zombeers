...
// Screens
export let roomSelectionScreen;
export let setupScreen;
export let gameScreen;
// Modals (using <dialog>)
export let optionsDialog;
export let endGameDialog;
// Room Selection Screen Elements
export let roomCodeInput;
export let joinRoomBtn;
export let createRoomBtn;
export let roomErrorMessage;
export let roomInfoBar;
...
export function initUI() {
    ...
    roomErrorMessage = $('#room-error-message');
    roomInfoBar = $('#room-info');
    ...
    leaveRoomBtnGame = $('#leave-room-btn-game');
    ...
    players.forEach(player => {
        ...
        card.innerHTML = `
            <div class="player-card-header">
                ...
                <button class="btn-reset-score" data-action="reset-score" data-player-id="${player.id}" aria-label="Reset score for ${escapeHTML(player.name)}">Reset Score</button>
            </div>
...
export function showNotification(message, type = 'info', duration = 3000) {
    if (!notificationArea) return; // Exit if the area isn't found

    const notification = document.createElement('div');
    notification.className = `notification ${type}`; // Add base and type class
